...
const endpoint = "player/detail?player_id=";
  ...
  axios.get(`http://localhost:8080/sportshubweb/${endpoint}${user_api_id}`).then(async (res) => {
    const person_data = res.data;

   await showInEdit(person_data.data);
...
async function showInEdit(person_data) {
  ...
  document.getElementById("user_image").value = person_data.Url;
	document.getElementById("user_password").value =  await person_data.Password;
